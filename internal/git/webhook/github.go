...
type GitHubProvider struct {
	...
	checkoutDir  string // carried for cohesion; used by the schedule's SyncOptions
	functionsDir string
	sshDir       string
}
...
func (h *GitHubProvider) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if ev != "push" {
		logf(h.logger, r.Context(), slog.LevelDebug, "webhook: ignoring event", "event", ev)
	...
	if err := json.Unmarshal(body, &payload); err != nil {
		logf(h.logger, r.Context(), slog.LevelWarn, "webhook: malformed push payload")
		http.Error(w, "bad request", http.StatusBadRequest)
		return
	}
	// A deleted push removes a ref; nothing to sync (there is no new tree).
	if payload.Deleted {
		logf(h.logger, r.Context(), slog.LevelDebug, "webhook: push deleted; nothing to sync", "repo", h.repoName(&payload))
		w.WriteHeader(http.StatusOK)
		_, _ = io.WriteString(w, "ignored")
		return
	}
	...
	if cfgErr != nil || !MatchRepository(cfg.Repository, &payload) {
		logf(h.logger, r.Context(), slog.LevelWarn, "webhook: repository mismatch", "repo", h.repoName(&payload))
		w.WriteHeader(http.StatusOK)
		_, _ = io.WriteString(w, "ignored")
		return
	}
	...
	if !git.MatchPushedRef(payload.Ref, cfg.Ref) {
		logf(h.logger, r.Context(), slog.LevelDebug, "webhook: pushed ref does not match configured ref",
	...
	if h.sync == nil {
		logf(h.logger, r.Context(), slog.LevelError, "webhook: sync trigger not configured")
		http.Error(w, "internal error", http.StatusInternalServerError)
		return
	}
	if err := h.sync.Trigger(); err != nil {
		logf(h.logger, r.Context(), slog.LevelError, "webhook: schedule sync: %v", err)
		http.Error(w, "internal error", http.StatusInternalServerError)
		return
	}
	logf(h.logger, r.Context(), slog.LevelInfo, "GitHub push accepted",
...
// non-matching), matching the "200 ignored" behavior. Exported for tests.
...
func (s *SyncScheduler) run() {
	for {
		...
		if s.logger != nil {
			if err != nil {
				s.logger.Error(fmt.Sprintf("Webhook: Git sync failed: %v", err))
			} else {
