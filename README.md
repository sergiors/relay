...
verification is never disabled**: sync verifies the host against the system
known_hosts files (`~/.ssh/known_hosts`, `/etc/ssh/ssh_known_hosts`, or the
`SSH_KNOWN_HOSTS` env var). If no known_hosts entry exists, sync fails with
guidance (`ssh-keyscan <host> >> ~/.ssh/known_hosts`) rather than proceeding
insecurely.
