...
- `runtime` selects the execution runtime. Only `python3.14` and `node24` are
  supported; any other value fails validation. It is **required** whenever Relay
   ...
   all use the `image` source needs no `runtime`. On `node24` a handler
...
- `services` (optional) is a list of persistent long-running HTTP services (see
  ...
  (an application entrypoint **file**, not the `module.function` form) or
  `image` (an external image reference) — plus optional `port` (default `80`,
...
- `image` is an **external image reference** (e.g. `nginx:1.27`,
  ...
  An `image` service does **not** need a `runtime:`.
- A template whose only services use the `image` source needs **no `runtime:` at
  all**. A **mixed** template (events or schedules, or any `entrypoint` service)
  still requires it, because those run through a runtime. An explicitly
  configured runtime is always validated, so a typo in an otherwise image-only
  template is still a parse error.
...
Both source kinds share **one cohesive service reconciler and lifecycle** —
...
image repository, so the existing image-retirement machinery covers it. External
images are never Relay-cleaned.
...
cannot be resolved (a failed pull, a missing local image, an
...
`relay.identity` is the configured source descriptor (entrypoint file or image
reference) — an honest identity for every source kind, never a synthetic
entrypoint. Service containers carry no
...
`PYTHONDONTWRITEBYTECODE=1` for Python; empty for image sources), then the
...
`relay function inspect` shows the effective values (defaults included). An
`image` source is rendered with its kind prefix:
...
- `<id>` is a single deterministic Traefik-safe router/service id shared by the
  ...
  function name + source descriptor — entrypoint file or image
...
- This is a handler-level transpile only for **event/schedule handlers**. A
  long-running service that needs a real frontend/build pipeline declares an
  `image:` source (an image built and published outside Relay), which carries its
  own ENTRYPOINT and is inspected/pulled by Relay (see _Services_).
...
Git lives under the fixed convention paths (mirroring the secrets/state layout);
the compose volume mount at `/var/lib/relay` persists them.

- `/var/lib/relay/ssh/id_ed25519` — the private deploy key (0700 dir, 0600 file,
  written atomically). Only ever generated by `relay git keygen`.
- `/var/lib/relay/ssh/known_hosts` — Relay's own host-key pins (0600), maintained
  by TOFU; see below.
- `/var/lib/relay/git/source.json` — the persisted sync config (0600).
- `/var/lib/relay/git/checkout` — the managed git clone/worktree. Sync deletes
  and re-clones it when the configured repository changes.

### Auth and host verification

Sync authenticates over SSH only, using the local ed25519 key. **Host-key
verification is never disabled** — but Relay does not read the operator's system
known_hosts and never needs `ssh-keyscan`. Instead it maintains its own
`/var/lib/relay/ssh/known_hosts` with a Trust On First Use (TOFU) policy,
working with GitHub, GitLab, or any SSH git server:

- The first sync reaches a host, it **trusts** that key, prints the fingerprint
  (`Trusted new host <host> (fingerprint SHA256:...)`), and persists it.
- Every later sync **verifies** against that pinned key.
- A changed host key **fails the sync** with a clear man-in-the-middle warning
  and is never auto-replaced — a genuinely rotated key is resolved by the
  operator editing `/var/lib/relay/ssh/known_hosts` and syncing again.

### Monorepos and determinism

`relay git sync` checks out the configured ref (detached HEAD, a hard reset — the
remote is always the source of truth; it never pulls, so no merge state is ever
produced), then locates the functions source at the repo root or the monitored
...
configured and synced. Each function is copied into /functions atomically-ish
(temp copy + rename), so the reconciler never sees a half-written function. A
valid checkout containing zero functions is allowed — sync succeeds and leaves
`/functions` empty.

### Status and remove

`relay git status` prints the configured source, whether the SSH key and checkout
exist, the resolved commit (if a checkout exists), and the last sync time — never
...
- `examples/functions/external-image-service/` (**no runtime**): a persistent
